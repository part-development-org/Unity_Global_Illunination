	...
	SubShader
	{
		...
		Pass
		{
			...
			//
				...
				occ.a = pow(occ.a, 2 + (1 - occ.a) * 3.1415926);				
		...
		Pass
		{
			...
			//
				...
				for (int s = 0; s < samplesCount; s++)
				{
					...
					float dist = max(0.005, _scale * noise.w);
		...
		Pass
		{
			...
			{			
				...
				//Blur
					...
					angle = pow(angle, 2.2);
					float thresh = lerp(0.1, 0.01, angle) * n0.w;

				float w0 = 0.2270270270 + 0.001;
				float w1 = compare(n0, getNormal(ao1.xyz, i.uv1.zw), thresh, 0.65) * 0.3162162162 + 0.001;
				float w2 = compare(n0, getNormal(ao2.xyz, i.uv1.xy), thresh, 0.65) * 0.3162162162 + 0.001;
				float w3 = compare(n0, getNormal(ao3.xyz, i.uv2.zw), thresh, 0.65) * 0.0702702703 + 0.001;
				float w4 = compare(n0, getNormal(ao4.xyz, i.uv2.xy), thresh, 0.65) * 0.0702702703 + 0.001;
		...
		Pass
		{
			...
			sampler2D _MainTex, _HalfRes, _SmallAO;
			uniform half4 _MainTex_TexelSize;
			...
			fixed4 fragAO (v2f i) : SV_Target
			{
				...
				float4 ao1 = tex2D(_SmallAO, i.uv);
